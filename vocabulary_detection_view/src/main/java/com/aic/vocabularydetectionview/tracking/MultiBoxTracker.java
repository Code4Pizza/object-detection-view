...
package com.aic.vocabularydetectionview.tracking;
...
public class MultiBoxTracker {
  ...
  private void processResults(final List<Detector.Recognition> results) {
    ...
    for (final Pair<Float, Detector.Recognition> potential : rectsToTrack) {
      ...
      trackedRecognition.croppedBitmap = potential.second.getCroppedBitmap();
  ...
  public TrackedRecognition locateFrameClicked(int x, int y) {
    for (Pair<Float, RectF> rect : screenRects) {
      if (rect.second.contains(x, y)) {
        for (final TrackedRecognition potential : trackedObjects) {
            if (rect.first.equals(potential.getDetectionConfidence())) {
              return potential;
            }
        }
      }
